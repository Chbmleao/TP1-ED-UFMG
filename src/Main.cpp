...
void executeRound(int numTotalPlayers, Round *round, List *allPlayers) {
    ...
    for (int j = 0; j < numTotalPlayers; j++) {
        ...
        // ESCREVEMEMLOG((long int)(allPlayers->getPlayer(j+1)->key), sizeof(long int), round->id);
    }
...
int main(int argc, char ** argv) {
    ...
    if (inputFile.is_open() && outputFile.is_open()) {
        ...
        for (int i = 0; i < numRounds; i++) {
            ...
            // ESCREVEMEMLOG((long int)(&(round.id)), sizeof(long int), round.id);
            ...
            for (int j = 0; j < numPlayers; j++) {
                ...
                for (int k = 0; k < 5; k++) {
                    ...
                    ESCREVEMEMLOG((long int)(&(player->hand.getItems()[0])), sizeof(Card), round.id);
                }
            }

            int numTotalPlayers = allPlayers.getSize();
            executeRound(numTotalPlayers, &round, &allPlayers);
