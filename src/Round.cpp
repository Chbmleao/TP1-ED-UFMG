...
void Round::insertPlayer(Player *player) {
    ...
    // ESCREVEMEMLOG((long int)(player), sizeof(Player*), this->id);
}
...
void Round::receiveBets() {
    for (int i = 0; i < this->numberOfPlayers; i++) {
        ...
        // ESCREVEMEMLOG((long int)(players.items[i]), sizeof(Player), this->id);
        // LEMEMLOG((long int)(players.items[i]), sizeof(Player), this->id);
    }
}
...
void Round::writeWinners(std::ofstream &outputFile) {
    ...
    for (int i = 1; i <= winners->getSize(); i++) {
        ...
        // LEMEMLOG((long int)(winners->getPlayer(i)), sizeof(Player), this->id);
    }
}
