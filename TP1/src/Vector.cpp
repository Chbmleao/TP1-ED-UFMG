...
int Vector<T>::accessVector() {
    ...
    for (int i=0; i<this->size; i++){
        aux = this->items[i];
        LEMEMLOG((long int)(&(this->items[i])),sizeof(T),this->key);
    }
    return s; 
}
...
void Vector<T>::writeElement(T item) {
    ...
    ESCREVEMEMLOG((long int)(&(this->items[this->position])),sizeof(T),this->key);
...
T Vector<T>::readElement (int pos) {
    ...
    LEMEMLOG((long int)(&(this->items[pos])),sizeof(T),this->key);
