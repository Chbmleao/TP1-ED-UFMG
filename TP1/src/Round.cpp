...
Round::Round(int numberOfPlayers, int minimumBet){
    ...
    erroAssert(minimumBet > 0 && minimumBet % 50 == 0, "Invalid minimum bet. Minimum bet must be bigger than 0 and multiple of 50.");
    this->minimumBet = minimumBet;
    this->totalAmmount = 0;
    this->players = Vector<Player*>(5);
...
int Round::breakTheTie(Player* player1, Player* player2) {
    ...
    switch (move){
        case HC:
            return this->auxBreakTheTie(player1, player2, HAND_SIZE-1);
            break;
        ...
        case OP: 
            ...
            if (max1 < max2) 
                ...
                return this->auxBreakTheTie(player1, player2, player1->getCardIndex(max1));
        ...
        case TP:
            ...
            if (max1 < max2) 
                ...
                if (min1 < min2) 
                    ...
                    return this->auxBreakTheTie(player1, player2, player1->getCardIndex(min1));
            }
            break;
        ...
        case FH:
            ...
            if (max1 < max2) 
                ...
                if (min1 < min2)
                    ...
                    this->auxBreakTheTie(player1, player2, player1->getCardIndex(min1));
            } 
            break;
...
void Round::getWinner() {
    ...
    while (i < this->winners->getSize()) {
        ...
        if (finalWinner == 1) {
            ...
            this->winners->removeAtPosition(i);
        } else {
            i++;
        }
    }
...
void Round::distributeAmmount() {
    double reward = (double)this->totalAmmount / (double)this->winners->getSize();
...
void Round::writeWinners(std::ofstream &outputFile) {
    ...
    outputFile << (double)this->getTotalAmmount()/(double)this->winners->getSize() << " ";
