...
Player::Player(std::string name, int money, int bet) {
    erroAssert(!name.empty(), "Name string is empty.");
    avisoAssert(bet >=0 && bet % 50 == 0, "Bet must be multiple of 50 and positive.");
...
int Player::decreaseMoney(int value) {
    
    if (this->finalMoney - value < 0) {
        avisoAssert(this->finalMoney - value >= 0, "Not enough money to decrease. The round will be canceled.");
        return 1; // if it isnt possible to decrease money
    }
    
    this->finalMoney -= value;
    return 0; // if it is possible to decrease money
}
